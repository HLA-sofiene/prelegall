...
export default function DocChat({ values, docType, onFieldsUpdate, onDocTypeDetected }: DocChatProps) {
  const [messages, setMessages] = useState<Message[]>([])
  const [input, setInput] = useState('')
  const [isLoading, setIsLoading] = useState(false)
  const messagesEndRef = useRef<HTMLDivElement>(null)
  const inputRef = useRef<HTMLTextAreaElement>(null)
  // Always reflect the latest values and docType so callApi never uses stale snapshots
  ...
  async function callApi(msgs: Message[]) {
    ...
    try {
      const res = await fetch('/api/chat', {
        ...
        body: JSON.stringify({
          messages: msgs,
          current_fields: valuesRef.current,
      ...
      if (data.doc_type && !docTypeRef.current) {
        // Update the ref synchronously so rapid follow-up calls don't double-fire detection
        docTypeRef.current = data.doc_type
        onDocTypeDetected(data.doc_type)
      }
  ...
  async function handleSend() {
    ...
    setMessages(updated)
    setInput('')
    // Focus immediately after clearing input so the user can keep typing
    inputRef.current?.focus()
